...
class PubgClient:
    ...
    async def get_ranked_report(self, names: list[str], game_mode: str) -> tuple[list[dict], list[str]]:
        ...
        # Do these one at a time.  Starting a task for every roster member
        # looks fast, but a PUBG 429 then leaves dozens of requests queued
        # behind the retry delay.  Sequential requests keep the queue small
        # and let one cooldown recover the entire report.
        for p in found:
    ...
    async def get_daily_activity_report(
        ...
        # Store in-flight tasks, not just completed values. Clan members
        # commonly share the same matches; without this, simultaneous player
        # processing can download and parse the same multi-megabyte telemetry
        # file several times before the first request reaches the cache.
        match_tasks: dict[str, asyncio.Task] = {}
        kill_breakdown_tasks: dict[str, asyncio.Task] = {}
        cache_lock = asyncio.Lock()
        # Telemetry files are large. On a small hosted container, loading
        # several at once can exceed the memory limit and kill the entire
        # Bot process (exit code 137). Process one at a time for stability.
        ...
        async def get_match(match_id: str) -> dict:
            async with cache_lock:
                ...
                if task is None:
                    task = asyncio.create_task(fetch_match(match_id))
                    match_tasks[match_id] = task
            return await task

        async def fetch_kill_breakdown(telemetry_url: str | None) -> dict[str, tuple[int, int, int]]:
            ...
            # Drop the potentially multi-megabyte telemetry event list
            # before this coroutine completes and another telemetry file is
            # allowed to load.
            if telemetry_url:
                del events
            return result
        ...
        async def get_kill_breakdown(match_id: str, telemetry_url: str | None) -> dict[str, tuple[int, int, int]]:
            async with cache_lock:
                task = kill_breakdown_tasks.get(match_id)
                if task is None:
                    task = asyncio.create_task(fetch_kill_breakdown(telemetry_url))
                    kill_breakdown_tasks[match_id] = task
            return await task
        ...
        async def process_player(p: dict):
            totals = {
                ...
                "team_kills": 0, "teamKills": 0, "stooge_kills": 0, "boosts": 0, "heals": 0,
